/* Weenie - Scroll of Mana Depletion Other II (3093) */
DELETE FROM weenie WHERE class_Id = 3093;

INSERT INTO weenie (`class_Id`, `class_Name`, `type`)
VALUES (3093, 'scrollmanadepletion2', /* Scroll_WeenieType */ 34);

INSERT INTO `weenie_properties_string` (`object_Id`, `type`, `value`)
VALUES (3093, 16, 'When learned, this spell decreases target''s natural mana rate by 33%.') /* LONG_DESC_STRING */
     , (3093, 1, 'Scroll of Mana Depletion Other II') /* NAME_STRING */
     , (3093, 15, 'A magic scroll.') /* SHORT_DESC_STRING */;

INSERT INTO `weenie_properties_d_i_d` (`object_Id`, `type`, `value`)
VALUES (3093, 1, 33554826) /* SETUP_DID */
     , (3093, 8, 100676939) /* ICON_DID */
     , (3093, 22, 872415275) /* PHYSICS_EFFECT_TABLE_DID */
     , (3093, 28, 219) /* SPELL_DID */;

INSERT INTO `weenie_properties_int` (`object_Id`, `type`, `value`)
VALUES (3093, 9, 0) /* LOCATIONS_INT */
     , (3093, 1, 8192) /* ITEM_TYPE_INT */
     , (3093, 93, 1044) /* PHYSICS_STATE_INT */
     , (3093, 5, 30) /* ENCUMB_VAL_INT */
     , (3093, 16, 8) /* ITEM_USEABLE_INT */
     , (3093, 8, 90) /* MASS_INT */
     , (3093, 19, 5) /* VALUE_INT */;

INSERT INTO `weenie_properties_float` (`object_Id`, `type`, `value`)
VALUES (3093, 39, 1.5) /* DEFAULT_SCALE_FLOAT */;

INSERT INTO `weenie_properties_bool` (`object_Id`, `type`, `value`)
VALUES (3093, 22, True) /* INSCRIBABLE_BOOL */
     , (3093, 23, True) /* DESTROY_ON_SELL_BOOL */;

