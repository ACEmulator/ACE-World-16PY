/* Weenie - Scroll of Axe Mastery Self III (3169) */
DELETE FROM ace_weenie_class WHERE weenieClassId = 3169;

INSERT INTO ace_weenie_class (`weenieClassId`, `weenieClassDescription`)
VALUES (3169, 'scrollaxemasteryself3');

INSERT INTO `ace_object` (`aceObjectId`, `aceObjectDescriptionFlags`, `weenieClassId`)
VALUES (3169, 18, 3169);

INSERT INTO `ace_object_properties_string` (`aceObjectId`, `strPropertyId`, `propertyValue`)
VALUES (3169, 16, 'When learned, this spell increases the caster''s Axe skill by 50%.') /* LONG_DESC_STRING */
     , (3169, 1, 'Scroll of Axe Mastery Self III') /* NAME_STRING */
     , (3169, 15, 'A magic scroll.') /* SHORT_DESC_STRING */;

INSERT INTO `ace_object_properties_did` (`aceObjectId`, `didPropertyId`, `propertyValue`)
VALUES (3169, 1, 33554826) /* SETUP_DID */
     , (3169, 8, 100676449) /* ICON_DID */
     , (3169, 22, 872415275) /* PHYSICS_EFFECT_TABLE_DID */
     , (3169, 28, 300) /* SPELL_DID */;

INSERT INTO `ace_object_properties_int` (`aceObjectId`, `intPropertyId`, `propertyValue`)
VALUES (3169, 9, 0) /* LOCATIONS_INT */
     , (3169, 1, 8192) /* ITEM_TYPE_INT */
     , (3169, 93, 1044) /* PHYSICS_STATE_INT */
     , (3169, 5, 30) /* ENCUMB_VAL_INT */
     , (3169, 16, 8) /* ITEM_USEABLE_INT */
     , (3169, 8, 90) /* MASS_INT */
     , (3169, 19, 20) /* VALUE_INT */
     , (3169, 9007, 34) /* Scroll_WeenieType */;

INSERT INTO `ace_object_properties_double` (`aceObjectId`, `dblPropertyId`, `propertyValue`)
VALUES (3169, 39, 1.5) /* DEFAULT_SCALE_FLOAT */;

INSERT INTO `ace_object_properties_bool` (`aceObjectId`, `boolPropertyId`, `propertyValue`)
VALUES (3169, 22, True) /* INSCRIBABLE_BOOL */
     , (3169, 23, True) /* DESTROY_ON_SELL_BOOL */;

